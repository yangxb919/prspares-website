...
import { ArrowRight, BatteryFull } from 'lucide-react';
import QuoteModal from '@/components/QuoteModal';
import { trackEvent } from '@/lib/analytics';

interface iPadBatteryCardProps {
  model: string;
  capacity: string;
  priceFrom: number;
  badge?: string;
...
export default function IPadBatteryCard({ model, capacity, priceFrom, badge, badgeColor = 'bg-blue-600' }: iPadBatteryCardProps) {
  const [open, setOpen] = useState(false);
  const productName = `${model} Battery`;
  ...
  return (
    <>
      <button
        type="button"
        onClick={() => {
          trackEvent('quote_cta_click', { event_label: `iPad Battery: ${model}` });
          setOpen(true);
        }}
      ...
      >
        ...
        <div className="p-4">
          <h4 className="font-bold text-gray-900 mb-1 text-sm">{model}</h4>
          <p className="text-xs text-gray-500 mb-3">{capacity}</p>
