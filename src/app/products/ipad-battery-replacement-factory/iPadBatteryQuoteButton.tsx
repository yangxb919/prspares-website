...
interface iPadBatteryQuoteButtonProps {
  label?: string;
  product?: string;
  eventLabel?: string;
...
export default function IPadBatteryQuoteButton({
  label = 'Request Quote',
  product = 'iPad Batteries',
  eventLabel = 'iPad Battery CTA',
  variant = 'primary',
  className = '',
}: iPadBatteryQuoteButtonProps) {
  const [open, setOpen] = useState(false);

  if (variant === 'footer-wa') {
    return (
      <a
        href={`https://wa.me/85363902425?text=Hi,%20I'm%20interested%20in%20wholesale%20iPad%20batteries`}
        target="_blank"
