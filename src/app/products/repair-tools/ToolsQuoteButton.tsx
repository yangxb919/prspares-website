...
interface ToolsQuoteButtonProps {
  label?: string;
  product?: string;
  eventLabel?: string;
...
export default function ToolsQuoteButton({
  label = 'Request Quote',
  product = 'Repair Tools',
  eventLabel = 'Repair Tools CTA',
  variant = 'primary',
  className = '',
}: ToolsQuoteButtonProps) {
  const [open, setOpen] = useState(false);

  if (variant === 'footer-wa') {
    return (
      <a
        href={`https://wa.me/85363902425?text=Hi,%20I'm%20interested%20in%20wholesale%20phone%20repair%20tools`}
        target="_blank"
