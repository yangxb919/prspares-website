...
interface CameraQuoteButtonProps {
  label?: string;
  product?: string;
  eventLabel?: string;
...
export default function CameraQuoteButton({
  label = 'Request Quote',
  product = 'iPhone Rear Cameras',
  eventLabel = 'iPhone Camera CTA',
  variant = 'primary',
  className = '',
}: CameraQuoteButtonProps) {
  const [open, setOpen] = useState(false);

  if (variant === 'footer-wa') {
    return (
      <a
        href={`https://wa.me/85363902425?text=Hi,%20I'm%20interested%20in%20wholesale%20iPhone%20rear%20cameras`}
        target="_blank"
